...
/* Free list of blocks the same size */
#define fwd	u.s.bhf
#define prev u.s.bhv

/* Free list of blocks of different sizes */
#define next u.s.bhn
...
pooldel(Pool *p, Bhdr *t)
{
    ...
    /* Block is being used but also pointed to by the sorted free list of sizes
     * need to take next element in the list and place it into the free list of sizes list */
	if(t->fwd != t) {
		f = t->fwd;
		s = t->parent;
		f->parent = s;
...
pooladd(Pool *p, Bhdr *q)
{
    ...
    /* t is not null due to previous check so no null issues */
    while(t->next){
        if(size == t->size){
            q->fwd = t;
            q->prev = t->prev;
        ...
        /* Only block of this size, add it to the list */
        if(size < t->size){
            q->next = t;
            q->next->parent = q;
            if(q->parent){
    ...
    /* We are adding a new block that is the largest
     * t is pointing at the last element in the list */
    q->parent = t;
    t->next = q;
...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
    ...
    /* Will hold the block of memory that is just larger than the size */
	q = nil;
	while(t && !q) {
		if(t->size == size) {
			t = t->fwd;
        ...
        if(size < t->size){
            /* We found a block larger than the size requested
             * so set it to q and break from the search loop */
            q = t;
            break;
        }
